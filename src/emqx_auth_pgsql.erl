...
check(mqtt, Password, Client = #mqtt_client{headers = Headers, username = Username} , {{AuthSql, AuthParams}, HashType}) ->
    ...
    case emqx_auth_pgsql_cli:equery(AuthSql, AuthParams, Client) of
        {ok, _, [{_TenantId, _ProductId, _GroupId, _DeviceId, _DeviceUsername, _Token, 1, _, _}]} ->
        ...
        {ok, _, [{TenantId, ProductId, GroupId, DeviceId, _, Token, _Status, 1, _}]} ->
...
mqtt_headers(TenantId, ProductId, GroupId, DeviceId, Headers) ->
    mqtt_headers(TenantId, ProductId, GroupId, DeviceId, 1, Headers).
mqtt_headers(TenantId, ProductId, GroupId, DeviceId, Type, Headers) ->
    [{tenant_id, TenantId},
     {product_id, ProductId},
     {group_id, GroupId},
     {device_id, DeviceId},
